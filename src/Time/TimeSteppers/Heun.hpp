...
class Heun : public RungeKutta {
 public:
  ...
  Heun() = default;
  Heun(const Heun&) = default;
  Heun& operator=(const Heun&) = default;
  Heun(Heun&&) = default;
  Heun& operator=(Heun&&) = default;
  ~Heun() override = default;
  ...
  WRAPPED_PUPable_decl_template(Heun);  // NOLINT

  explicit Heun(CkMigrateMessage* /*unused*/) {}

 private:
  const ButcherTableau& butcher_tableau() const override;
};

inline bool constexpr operator==(const Heun& /*lhs*/, const Heun& /*rhs*/) {
  return true;
}
