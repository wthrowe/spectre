 ...
 * the presence of jumps larger than floating point precision.
 */
template <typename F>
void test_positivity_with_roundoff(const size_t stencil_width,
                                   const F& invoke_recons) {
  const size_t num_points = 4;
  const Mesh<1> mesh{num_points, Spectral::Basis::FiniteDifference,
  ...
  const double small_value = 1.0e-15;
  const double large_value = 1.0e7;
  for (size_t i = 0; i < mesh.number_of_grid_points(); ++i) {
    const double x = get<0>(logical_coords)[i];
    var[i] = x > 0.0 ? (x - 0.2) * large_value : small_value;
  }
