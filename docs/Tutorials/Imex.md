...
is likely to be difficult.  Implicit integrators are therefore usually used
with a numerical solve of the update equation.  Even for simple systems, this
solution adds significantly to the computational cost of the method, and for a
system like a large PDE evolution it is intractable.
...
fluid variables will not be in any sector.
...
SpECTRE supports analytic solution of the implicit-step equation as well as two
modes for a numerical solution: implicit and semi-implicit.  The analytic mode
can be chosen in the definition of the implicit sector if the form of the
implicit source allows an analytic solution.  The numerical methods can be
toggled at runtime.  The fully implicit method does a numerical root-find,
while the semi-implicit one linearizes the equation and solves that.
...
In the executable itself, three sets of changes are necessary.  First, the
...
(The `TimeStepper` line can be removed, although doing so is not strictly
necessary.)
...
Finally, the `imex::ImplicitDenseOutput<system>` dense output postprocessor
must be added to the argument of the
`evolution::Actions::RunEventsAndDenseTriggers` action.
